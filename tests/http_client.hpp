...
{
public:
	using connect_t = tt_program::http_session::connect_t;

	using http_method = tt_program::http_session::http_method;

	using response_callback_t = tt_program::http_session::response_callback_t;

	using status = tt_program::http_session::status;

	using ssl = tt_program::http_session::ssl;
...
private:
	...
	std::shared_ptr<tt_program::http_session> find_http_session(ssl::disabled_ssl) const;
	std::shared_ptr<tt_program::http_session> find_http_session(ssl::enabled_ssl) const;

	std::shared_ptr<tt_program::http_session> create_http_session(ssl::disabled_ssl);
	std::shared_ptr<tt_program::http_session> create_http_session(ssl::enabled_ssl);

	void map_session(std::shared_ptr<tt_program::http_session> session);
...
private:
	...
	using sessions_map = std::unordered_map<std::string, std::shared_ptr<tt_program::http_session>>;
